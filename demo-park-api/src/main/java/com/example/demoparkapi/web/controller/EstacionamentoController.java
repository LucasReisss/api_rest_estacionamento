...
import com.example.demoparkapi.entity.ClienteVaga;
import com.example.demoparkapi.jwt.JwtUserDetails;
import com.example.demoparkapi.repository.projection.ClienteVagaProjection;
import com.example.demoparkapi.service.ClienteService;
import com.example.demoparkapi.service.ClienteVagaService;
import com.example.demoparkapi.service.JasperService;
import com.example.demoparkapi.web.dto.EstacionamentoCreateDto;
import com.example.demoparkapi.web.dto.EstacionamentoResponseDto;
import com.example.demoparkapi.web.dto.EstacionamentoService;
import com.example.demoparkapi.web.dto.PageableDto;
import com.example.demoparkapi.web.dto.mapper.ClienteVagaMapper;
...
public class EstacionamentoController {
    ...
    @Operation(summary = "Operação de check-in", description = "Recurso para dar entrada de um veículo no estacinoamento. " +
    ...
    @GetMapping("/cpf/{cpf}")
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<PageableDto> getAllEstacionamentos(@PathVariable String cpf,
    ...
    public ResponseEntity<PageableDto> getAllEstacionamentosDoCliente(@AuthenticationPrincipal JwtUserDetails user,
                                                                      @PageableDefault(size = 5, sort = "dataEntrada",
                                                                     direction = Sort.Direction.ASC)Pageable pageable) {
        Page<ClienteVagaProjection> projection = clienteVagaService.buscarTodosPorUserioId(user.getId(), pageable);
        PageableDto dto = PageableMapper.toDto(projection);
        return ResponseEntity.ok(dto);
    }
